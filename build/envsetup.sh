...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/MinusOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.minusos.device | grep -q "$MINUS_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/MinusOS-AOSP/$PROJECT
...
function installrecovery()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.minusos.device | grep -q "$MINUS_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.minusos.device | grep -q "$MINUS_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/minus/build/tools/repopick.py $@
}
